...
def find_best_nb_of_features(v=None,c=None,clf=None):
    ...
    if v=="vect":
        for ci in range(300,c,2):
            vect=CountVectorizer(max_features=ci,ngram_range=(1,2),stop_words='english',analyzer=split_into_stem)
        ...
        for ci in range(1,c,2):
            vect=TfidfVectorizer(stop_words="english",max_features=ci,ngram_range=(1,3),analyzer=split_into_stem)
            scores.append([test_score(vect,clf),ci])
        scores=[score for score in scores if score==max(scores)]
        print "maximum attainable score and features: ",scores
    else:
...
print"**********Naive Bayes***************"
find_best_nb_of_features("vect",5827,clfNB)
find_best_nb_of_features("tfidf",5827,clfNB)
print"**********Linear SVM*****************"
find_best_nb_of_features("vect",5827,linear_svc)
find_best_nb_of_features("tfidf",5827,linear_svc)
print"**********LogisticRegression*****************"
find_best_nb_of_features("vect",5827,logreg)
find_best_nb_of_features("tfidf",5827,logreg)
print"**********knn*****************"
find_best_nb_of_features("vect",5827,knn)
find_best_nb_of_features("tfidf",5827,knn)